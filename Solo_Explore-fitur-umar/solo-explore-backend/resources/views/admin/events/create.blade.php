...
<div class="bg-white rounded-lg shadow p-6">
    <form action="{{ route('admin.events.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-3">
                    <div class="flex gap-4">
                        <label class="flex items-center">
                            <input type="radio" name="image_type" value="upload" checked class="mr-2" onchange="toggleImageInputEvent()">
                            Upload File
                        </label>
                        <label class="flex items-center">
                            <input type="radio" name="image_type" value="url" class="mr-2" onchange="toggleImageInputEvent()">
                            URL Gambar
                        </label>
                    </div>
                    <input type="file" name="image" id="image_upload_event" accept="image/*"
                        class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                    <input type="url" name="image_url" id="image_url_event" placeholder="https://example.com/image.jpg"
            ...
            <script>
            function toggleImageInputEvent() {
                const imageType = document.querySelector('input[name="image_type"]:checked').value;
                const uploadInput = document.getElementById('image_upload_event');
                const urlInput = document.getElementById('image_url_event');
